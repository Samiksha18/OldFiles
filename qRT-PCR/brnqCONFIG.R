...
######  To use, edit all the info above the 4 lines as needed.  
#       Standard plates should be run portrait, not landscape
#       NOTE:  This requires the use of one (and only one) 
#       control primer set for normalization.  
#       qGot two?  
#       Re-run the analysis with the other set.
...
#################   Experiment title        #####################
experiment_title<-"brnQ_ilvD_Coldshock"  #@@@@@@@@@@@@@@@@@@@@@@@@@@
experiment_date<-"20150501"               #@@@@@@@@@@@@@@@@@@@@@@@@@@
...
###  Need to override threshold for calculating Cq's?  Uncomment the line below
...
inputDirStdNorm<-"../../Lab_data/RT_PCR/stdCurvesSYBR20150408/"   #@@@@@@@@@@@@@@@@@@@@@@@@@@
inputDirStdTest<-"../../Lab_data/RT_PCR/brnqstd/"  #@@@@@@@@@@@@@@@@@@@@@@@@@@
inputDirSamp<-"../../Lab_data/RT_PCR/brnq_ilvd_rpoc/"   #@@@@@@@@@@@@@@@@@@@@@@@@@@
...
#################   Standard Curve of Norm Primer Inputs  #####################
filesStdNorm<-grep("^\\~", list.files(inputDirStdNorm, all.files = F), value = T, invert = T)  ##  this should get rid of ~$ temps
standard_cqwk<- loadWorkbook(paste(inputDirStdNorm,  as.character(grep("Quantification Cq Results.xlsx", filesStdNorm, value = T)), sep=""))
standard_cqdf<- readWorksheet(standard_cqwk, sheet="0")
standard_ampwk<- loadWorkbook(paste(inputDirStdNorm,  as.character(grep("Quantification Amplification Results.xlsx", filesStdNorm, value = T)), sep=""))
######  quick clean
...
filesStdTest<-grep("^\\~", list.files(inputDirStdTest, all.files = F), value = T, invert = T)  ##  this should get rid of ~$ temps
testStandard_cqwk<- loadWorkbook(paste(inputDirStdTest,  as.character(grep("Quantification Cq Results.xlsx", filesStdTest, value = T)), sep=""))
testStandard_cqdf<- readWorksheet(standard_cqwk, sheet="0")
testStandard_ampwk<- loadWorkbook(paste(inputDirStdTest,  as.character(grep("Quantification Amplification Results.xlsx", filesStdTest, value = T)), sep=""))
######  quick clean
testStandard_cqdf$row<-gsub("(\\D)(.*)", "\\1", testStandard_cqdf$Well)
testStandard_cqdf$col<-as.numeric(gsub("(\\D)(.*)", "\\2", testStandard_cqdf$Well))
testStandard_cqdf$well<-paste(testStandard_cqdf$row, testStandard_cqdf$col, sep="")
testStandard_cqdf<-testStandard_cqdf[complete.cases(testStandard_cqdf),]
...
try(ps2df<-readWorksheet(testStandard_ampwk, sheet=ps2), silent = T)
...
try(ps3df<-readWorksheet(testStandard_ampwk, sheet=ps3), silent = T)
...
try(ps4df<-readWorksheet(testStandard_ampwk, sheet=ps4), silent = T)
...
try(ps5df<-readWorksheet(testStandard_ampwk, sheet=ps5), silent = T)
...
try(ps6df<-readWorksheet(testStandard_ampwk, sheet=ps6), silent = T)
