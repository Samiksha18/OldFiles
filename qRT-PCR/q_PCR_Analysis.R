...
write.csv(sumdf, paste(new_path,title, "_std_calculated_Cq.csv", sep=""))
...
try(all$temp<-gsub("(.*)\\((.*)\\)(.*)", "\\2", all$name), silent=T)
all<-all[all$ps != "",]
...
summ<-subset(summ, condition !="Water")# "Neg Ctrl")

## Biological replicates
summ<-summ %>%group_by(str, temp, ps,condition) %>% mutate(bio_Cq_mean= mean(tech_Cq_mean))
summ<-summ %>%group_by(str, temp, ps,condition) %>% mutate(bio_Cq_StDev= sd(tech_Cq_mean))
summ<-summ %>%group_by(str, temp, ps,condition) %>% mutate(bio_Cq_StErrMean= sd(tech_Cq_mean)/sqrt(length(tech_Cq_mean)))
#summ$name<-paste(summ$str, "(", summ$temp,")", sep="")
summ[is.na(summ)] <- 0
...
norm1_bar<-ggplot(backsum, aes(x=name, y=as.numeric(transcripts), color=as.character(temp), fill=str))+
  geom_bar(stat="identity")+
  geom_errorbar(aes(ymin=as.numeric(transcripts)-transcript_error, ymax=as.numeric(transcripts)+transcript_error), color="black")+
  scale_y_log10()+
