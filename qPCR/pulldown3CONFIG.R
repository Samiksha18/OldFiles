...
inputDirStd<-"../../Lab_data/pulldown3/pulldown_std/"   #@@@@@@@@@@@@@@@@@@@@@@@@@@
inputDirSamp<-"../../Lab_data/pulldown3/pulldown3a/"   #@@@@@@@@@@@@@@@@@@@@@@@@@@
inputDirSamp2<-"../../Lab_data/pulldown3/pulldown3b/"   #@@@@@@@@@@@@@@@@@@@@@@@@@@
#inputDirSamp3<-"../../Lab_data/RT_PCR/brnq/"   #@@@@@@@@@@@@@@@@@@@@@@@@@@
#inputDirSamp4<-"../../Lab_data/RT_PCR/brnq/"   #@@@@@@@@@@@@@@@@@@@@@@@@@@


####  make a new directory
new_path<-paste("~/GitHub/R/qPCR/", title, "/",sep="")
dir.create(paste(gsub("(.*)/", "\\1", new_path)), showWarnings = TRUE, recursive = FALSE)

######## What did the nanodrop of your genomic DNA read, and what was the resulting copies/ul? #########
inp_dna_concentration<-1#1.82*10^8
...
standard_cqdf<-standard_cqdf[complete.cases(standard_cqdf),]  #  Whoa, calm down now!
...
# psadf<-readWorksheet(standard_ampwk, sheet="SYBR")
# psadf$ps<-ps1
# psadf<-melt(psadf, na.rm = T, value.name = "val", id.vars = c("Cycle", "ps"), variable.name = "well")
# psadf$rep_group<-gsub("(\\D)(.+)", "\\1", psadf$well)
...
all<-suppressWarnings(rbind(try(get(workbookList[1]), silent =T ), 
