...
#library(RUVSeq)
#library(EDASeq)
filter <- apply(countsTable, 1, function(x) length(x[x>5])>=2)
...
#####################################  RUVs  ###################
differences <- matrix(data=c(1:2, 3:4, 5:6,7:8 ),byrow=F, nrow=4)#,  9:10, 11:12), byrow=TRUE, nrow=6)
differences
...
countsTable2<-counts(get(method))
###############################################################################
### Make DESeq Data Set
...
pData0 = data.frame(cbind(samples, condition))
...
ddsfm <- DESeqDataSetFromMatrix(countData = countsTable, colData=pData0, design=~conditions)
ddsfm1 <- DESeqDataSetFromMatrix(countData = countsTable, colData=pData, design= ~ RUV+conditions)
...
WT<-c[grep(wtTerm, c, invert = F)]
#dd<-c[grep("back.WT", c, invert = F)]
print("non-WT strain list:")
print(cc)
#resList<-paste(cc, "res", sep="")

try(aRes<-results(dds, contrast=c("conditions", cc[5], WT)), silent=T)
try(bRes<-results(dds, contrast=c("conditions", cc[4], WT)), silent=T)
try(cRes<-results(dds, contrast=c("conditions", cc[3], WT)), silent=T)
try(dRes<-results(dds, contrast=c("conditions", cc[1], WT)), silent=T)
eRes<-results(dds, contrast=c("conditions", cc[2], WT)) # null

# aRes1<-results(dds2, contrast=c("condition", cc[5], WT))
# bRes1<-results(dds2, contrast=c("condition", cc[4], WT))
# cRes1<-results(dds2, contrast=c("condition", cc[3], WT))
# dRes1<-results(dds2, contrast=c("condition", cc[1], WT))
# eRes1<-results(dds2, contrast=c("condition", cc[2], WT))
...
data <- plotPCA(rlog(dds), returnData=TRUE)
...
################  Histogram of pvals
facet_hist<-ggplot(resdf, aes(resdf$padj))+ geom_histogram(binwidth=.05)+facet_wrap(~comp)+scale_y_log10()
facet_hist
facet_hist_trim<-ggplot(resdf[resdf$baseMean > 1,], aes(resdf$pvalue[resdf$baseMean > 1]))+ geom_histogram(binwidth=.05)+facet_wrap(~comp)+scale_y_log10()
facet_hist_trim
...
####### Sig Genes
sigTable<- filter(statTable, eRes$padj<pval_thresh)

try(de_a<- filter(statTable, abs(aRes$log2FoldChange)>fcl2), silent=T)
...
de_e<- filter(statTable, abs(eRes$log2FoldChange)>fcl2)
...
###############  tight
try(tde_a<- filter(statTable, abs(aRes$log2FoldChange)>fcl2tight), silent=T)
...
tde_e<- filter(statTable, abs(eRes$log2FoldChange)>fcl2tight)
...
altDifAndSig<-intersect(de_e$gene, sigTable$gene)                   ###### Best of Both, pessimistic
...
print(paste("Wild Type is:",  wildType))   ####   Time 
print(paste("null is:",  nullCodY))        ####      to
...
rpkmosdf$reg <- ifelse(rowMeans(rpkmosdf[unlist(i[nullCodY])])>rowMeans(rpkmosdf[unlist(i[wildType])]), "Down", "Up")   
head(rpkmosdf)
...
downreg$reg<-NULL
upreg$reg<-NULL
downreg[paste(wildType, "_contrast", sep="")]<-((rowMeans(downreg[unlist(i[wildType])])- rowMeans(downreg[unlist(i[wildType])]))/
                                                  (rowMeans(downreg[unlist(i[ nullCodY])])- rowMeans(downreg[unlist(i[wildType])])))*100
downreg[paste( nullCodY, "_contrast", sep="")]<-((rowMeans(downreg[unlist(i[nullCodY])])- rowMeans(downreg[unlist(i[wildType])]))/
                                                   (rowMeans(downreg[unlist(i[ nullCodY])])- rowMeans(downreg[unlist(i[wildType])])))*100
downreg[paste(st1, "_contrast", sep="")]<-((rowMeans(downreg[unlist(i[st1])])- rowMeans(downreg[unlist(i[wildType])]))/
                                                (rowMeans(downreg[unlist(i[ nullCodY])])- rowMeans(downreg[unlist(i[wildType])])))*100
downreg[paste(st2, "_contrast", sep="")]<-((rowMeans(downreg[unlist(i[st2])])- rowMeans(downreg[unlist(i[wildType])]))/
                                                (rowMeans(downreg[unlist(i[ nullCodY])])- rowMeans(downreg[unlist(i[wildType])])))*100
downreg[paste(st3, "_contrast", sep="")]<-((rowMeans(downreg[unlist(i[st3])])- rowMeans(downreg[unlist(i[wildType])]))/
                                                (rowMeans(downreg[unlist(i[ nullCodY])])- rowMeans(downreg[unlist(i[wildType])])))*100
downreg[paste(st4, "_contrast", sep="")]<-((rowMeans(downreg[unlist(i[st4])])- rowMeans(downreg[unlist(i[wildType])]))/
                                                (rowMeans(downreg[unlist(i[ nullCodY])])- rowMeans(downreg[unlist(i[wildType])])))*100

## up
upreg[paste(wildType, "_contrast", sep="")]<-((rowMeans(upreg[unlist(i[wildType])])- rowMeans(upreg[unlist(i[nullCodY])]))/
                                                (rowMeans(upreg[unlist(i[wildType])])- rowMeans(upreg[unlist(i[nullCodY])])))*100
upreg[paste( nullCodY, "_contrast", sep="")]<-((rowMeans(upreg[unlist(i[nullCodY])])- rowMeans(upreg[unlist(i[nullCodY])]))/
                                                (rowMeans(upreg[unlist(i[wildType])])- rowMeans(upreg[unlist(i[nullCodY])])))*100
upreg[paste(st1, "_contrast", sep="")]<-((rowMeans(upreg[unlist(i[st1])])- rowMeans(upreg[unlist(i[nullCodY])]))/
                                                (rowMeans(upreg[unlist(i[wildType])])- rowMeans(upreg[unlist(i[nullCodY])])))*100
upreg[paste(st2, "_contrast", sep="")]<-((rowMeans(upreg[unlist(i[st2])])- rowMeans(upreg[unlist(i[nullCodY])]))/
                                                (rowMeans(upreg[unlist(i[wildType])])- rowMeans(upreg[unlist(i[nullCodY])])))*100
upreg[paste(st3, "_contrast", sep="")]<-((rowMeans(upreg[unlist(i[st3])])- rowMeans(upreg[unlist(i[nullCodY])]))/
                                                (rowMeans(upreg[unlist(i[wildType])])- rowMeans(upreg[unlist(i[nullCodY])])))*100
upreg[paste(st4, "_contrast", sep="")]<-((rowMeans(upreg[unlist(i[st4])])- rowMeans(upreg[unlist(i[nullCodY])]))/
                                               (rowMeans(upreg[unlist(i[wildType])])- rowMeans(upreg[unlist(i[nullCodY])])))*100
...
server<-function(input, output) {
    ...
    output$volcanoPlot<-renderPlot({
      ...
      xlm<-c(-5, 5)
      ylm<-c(0, 10)
      eRes$threshold = as.factor((abs(eRes$log2FoldChange) > fcl2) & (eRes$padj < pval_thresh/no_of_genes))
      df<-data.frame(eRes)
      df<-na.omit(df)
