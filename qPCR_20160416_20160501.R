...
if(DEBUG){
  args<-c("~/20160502_10CYCLES_aggregated.csv",
          "~/qPCR_20160416/qpcr_stds.csv",
          "split", 
          "Adapters","", "Stock")
} else{
  args<-commandArgs(T)
...
for (i in c(normlist)){
  normalizer<-merged %>%
    ...
    mutate(norm_stock_per=transcripts_normalized_by_Adapters) %>%
...
#magick
 (merged$enrichment_ratio<-(merged$transcripts_normalized_by_Adapters)*
   (merged$norm_stock_per_Adapters))
#(merged$enrichment_ratio<-(merged$target_per_Adapters_output_input_ratio))

### quick and dirty
merged$strain<-factor(merged$strain,levels = c( "0", "0.4","2","10", "50","250","500", "Stock") )
...
#    rep!="B",
    ...
    !Target %in% c( "ilvD_CBS_u","icaB_CBS_u","rpoC"))
...
ggplot(merged[!merged$Target %in% c("rpoC", "Adapters") &
                !merged$strain %in% c("Stock"),],
       aes(x=strain, y=transcripts_normalized_by_Adapters))+geom_bar(stat="identity")+
