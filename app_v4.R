...
x<-relevel(x, "back.WT")
#x<-relevel(x, "WT")
set4 <- newSeqExpressionSet(as.matrix(DEsubset),
                           phenoData = data.frame(x, row.names=colnames(DEsubset), conditions=x))


design <- model.matrix(~x)
y <- DGEList(counts=counts(set4), group=x)
y <- calcNormFactors(y, method="upperquartile")
y <- estimateGLMCommonDisp(y, design)
y <- estimateGLMTagwiseDisp(y, design)
...
#head(res)
set4<-RUVr(set4, nonDE, k=1, residuals=res, center=T, round=TRUE, epsilon=1, tolerance=1e-8)
...
plotCounts(dds, gene=which.max(results(dds)[,"log2FoldChange"]), intgroup="conditions")
plotCounts(dds0, gene=which.max(results(dds0)[,"log2FoldChange"]), intgroup="conditions")

####  THE JUGGERNAUT   ("bum bum bummmmmmmm")  #####
extracted<-data.frame()
for (x in row.names(dds0[1:10])){
  y<-(plotCounts(dds0, gene=x,  intgroup="conditions", returnData=T))
...
extract_from_dds<-function(ii){
  output<-data.frame()
  for (x in row.names(ii[1:10])){
    y<-(plotCounts(ii, gene=x,  intgroup="conditions", returnData=T))
    z<-data.frame(t(data.frame(bbb=y$count, row.names=row.names(y))))
    row.names(z)[grep("^bbb$", row.names(z))]<-paste(x)
