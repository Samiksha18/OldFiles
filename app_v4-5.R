...
set4Test<-grep("no\\.WT", colnames(countsTable), value=T)
#set4Test<-grep("null", colnames(countsTable), value=T)
prex<-c(set4WT, set4Test)
#seq <- set
# get control genes
DEsubset<-countsTable[,prex]
...
top <- topTags(lrt,n=nrow(set), p.value = 1)$table
top<-top[with(top, order(row.names(top))),]
bottom <-top[!2^top$logFC >=fc_thresh | !2^top$logFC <=(1/fc_thresh) & !top$FDR<=pval_thresh,]
topl   <-top[ 2^top$logFC >=fc_thresh |  2^top$logFC <=(1/fc_thresh) &  top$FDR<=pval_thresh,]
...
x <- as.factor(rep(unique(gsub("(.*)(_.*)", "\\1", colnames(DEsubset))), each=techReps))
x<-relevel(x, "back.WT")
#x<-relevel(x, "WT")
set4e <- newSeqExpressionSet(as.matrix(DEsubset),
                            phenoData = data.frame(x, row.names=colnames(DEsubset), conditions=x))


design <- model.matrix(~x)
y <- DGEList(counts=counts(set4e), group=x)
y <- calcNormFactors(y, method="upperquartile")
y <- estimateGLMCommonDisp(y, design)
y <- estimateGLMTagwiseDisp(y, design)
...
#head(res)
set4<-RUVr(set4e, nonDE, k=1, residuals=res, center=T, round=TRUE, epsilon=1, tolerance=1e-8)
pData(set4<-RUVr(set4e, nonDE, k=1, residuals=res, center=T, round=TRUE, epsilon=1, tolerance=1e-8))
...
differences <- matrix(data=c(1:2, 3:4),byrow=T, nrow=2)

set3 <- RUVs(set4e, genes, k=1, differences)
pData(set3)
...
#
pData<-pData(set4)
pData$W_1<-2^(pData$W_1)
pData

#colnames(pData)<-c("samples", "conditions", "RUV")
#pData$scaled<-pData$RUVg/max(pData$RUVg)
ddsfm <- DESeqDataSetFromMatrix(countData = DEsubset, colData=pData0, design=~conditions)
ddsfm1 <- DESeqDataSetFromMatrix(countData = DEsubset, colData=pData, design= ~ W_1+conditions)
#design(ddsfm1) <- ~ pData$RUVg + condition
dds<-DESeq(ddsfm1, betaPrior = betaPriorBool)
dds0<-DESeq(ddsfm, betaPrior = betaPriorBool)
#normalizationFactors(dds)<-as.matrix(rescont[,1]) / exp(rowMeans(log(as.matrix(rescont[,1]))))
...
###  This is pure evil
# colData(dds)$sizeFactor<-colData(dds)$W_1
# colData(dds)$W_1<-NULL
# colData(dds0)$sizeFactor<-colData(dds)$W_1

###  ya see what i did?  Ohhh  Juuuugggernaut....!!!
plotCounts(dds, gene=which.max(results(dds)[,"log2FoldChange"]), intgroup=c("conditions"))
plotCounts(dds0, gene=which.max(results(dds0)[,"log2FoldChange"]), intgroup="conditions")
plotCounts2(dds, gene=which.max(results(dds)[,"log2FoldChange"]), intgroup=c("conditions"))
plotCounts2(dds0, gene=which.max(results(dds0)[,"log2FoldChange"]), intgroup="conditions")

######

####  THE JUGGERNAUT   ("bum bum bummmmmmmm")  #####
extracted<-data.frame()
for (x in row.names(dds[1:10])){
  y<-(plotCounts(dds, gene=x,  intgroup="conditions", returnData=T))
...
key<-data.frame(comp=prebind, cond=c(test1Term, try(as.character(cd), silent=T)))
key
resdf<-merge(resdf, key, all.x=T, by="comp")

#control means--  fold change from control means
fcfcm<-data.frame(control=rowMeans(DEsubset[,grep(control, colnames(DEsubset))]))
fcfcm$aREs<-(fcfcm$control*aRes$fc)
fcfcm$col<-(DEsubset$)
...
facet_MA<-ggplot(as.data.frame(resdf),aes(baseMean,log2FoldChange))+
  geom_point(aes(colour=((fc>3 |fc<.3) & padj<.01), size=-log(padj, 10)),alpha=0.9)+
...
write.csv(newdf, paste("",difsig_handle, sep=""))
...
preReadsPer<-as.data.frame(extracted)
...
try(readsPer<-readsPer %>% mutate(R05=unlist((10^9*readsPer[pre_rpkm[5]])/(as.numeric(sum(readsPer[pre_rpkm[5]]))*readsPer["len"]))),silent=T)
try(readsPer<-readsPer %>% mutate(R06=unlist((10^9*readsPer[pre_rpkm[6]])/(as.numeric(sum(readsPer[pre_rpkm[6]]))*readsPer["len"]))),silent=T)
try(readsPer<-readsPer %>% mutate(R07=unlist((10^9*readsPer[pre_rpkm[7]])/(as.numeric(sum(readsPer[pre_rpkm[7]]))*readsPer["len"]))),silent=T)
try(readsPer<-readsPer %>% mutate(R08=unlist((10^9*readsPer[pre_rpkm[8]])/(as.numeric(sum(readsPer[pre_rpkm[8]]))*readsPer["len"]))),silent=T)
try(readsPer<-readsPer %>% mutate(R09=unlist((10^9*readsPer[pre_rpkm[9]])/(as.numeric(sum(readsPer[pre_rpkm[9]]))*readsPer["len"]))),silent=T)
try(readsPer<-readsPer %>% mutate(R10=unlist((10^9*readsPer[pre_rpkm[10]])/(as.numeric(sum(readsPer[pre_rpkm[10]]))*readsPer["len"]))),silent=T)
try(readsPer<-readsPer %>% mutate(R11=unlist((10^9*readsPer[pre_rpkm[11]])/(as.numeric(sum(readsPer[pre_rpkm[11]]))*readsPer["len"]))),silent=T)
try(readsPer<-readsPer %>% mutate(R12=unlist((10^9*readsPer[pre_rpkm[12]])/(as.numeric(sum(readsPer[pre_rpkm[12]]))*readsPer["len"]))),silent=T)
...
ordering<-c("back.WT_1_RPKM", "back.WT_2_RPKM",
            "no.WT_1_RPKM", "no.WT_2_RPKM") 
cols2
